...
Pass an array of `sources` instead of a single `src` to ship several formats and let the browser pick the first one it can play — the standard cross-browser / adaptive-delivery mechanism. Each entry renders as a native `<source>` element with an optional `type` (MIME, lets the browser skip formats it can't decode) and `media` (a media query for resolution-based selection).
...
<Video
  sources={[
    ...
    { src: '/clip-sd.mp4', type: 'video/mp4', media: '(max-width: 600px)' },
  ]}
  fallbackSrc="/clip.mp4"
/>
```

`fallbackSrc` mirrors Mantine `Image`: it is loaded once if every `sources` entry fails at runtime (404, decode error). `sources` takes precedence over `src` when non-empty.

<Demo data={demos.sources} />

## Captions and subtitles

Pass `tracks` to attach WebVTT text tracks. Each entry renders a native `<track>` inside the
`<video>` element, so the browser paints the cues over the video itself and the CC button appears in
the control bar.
...
`kind` defaults to `captions`. Use `captions` for viewers who cannot hear the audio — they include
non-speech sound such as `[door slams]` — and `subtitles` for a translation of the dialogue. `default`
starts a track visible without the user turning captions on. `descriptions`, `chapters` and
`metadata` are accepted too.

This is the only way to attach tracks: `children` render **outside** the media element, because that
slot is the control bar, so a `<track>` placed there would never reach the `<video>`.

<Demo data={demos.captions} />

Captions are what makes `<Video.CaptionsButton />` appear — it hides itself when the player has no
`captions` or `subtitles` track to toggle.
...
> **See it live**
>
> → [Open the **fullscreen** demo page](https://gfazioli.github.io/mantine-video/fullscreen) → [Open the **homepage-style** demo page](https://gfazioli.github.io/mantine-video/homepage). Both pages live outside the docs layout, full‑bleed.
